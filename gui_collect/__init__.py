...
class App(tk.Tk):
    ...
    def create_widgets(self):
        self.mt_paned_window = tk.PanedWindow(
            self,
            orient=tk.VERTICAL,
            opaqueresize=True,
            showhandle=False,
        ...
        self.main = Main(
            parent=self.mt_paned_window, active_page=self.state.active_page
        )
    ...
    def grid_widgets(self):
        ...
        self.update()
        self.after_idle(
            lambda: self.mt_paned_window.sash_place(
                0, 0, int(self.winfo_height() * 0.75)
            )
        )
    ...
    def configure_style(self):
        ...
        self.style.map("Vertical.TScrollbar", background=[("active", "#333")])
