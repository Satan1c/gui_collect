...
class PathPicker(tk.Frame):
    def __init__(
        ...
        pass_self_to_callback=False,
        editable=False,
        editable_label_text=None,
        text_fg="#999",
        hover_text_fg="#e8eaed",
        override_open=False,
        font=None,
        ...
        **kwargs,
        ...
        if self.cfg_key_path:
            ...
            self.path = Path(
                self.cfg.get_config_key_value(self.cfg_key_path)
            ).absolute()
    ...
    def handle_click_change_path(self, event):
        path = filedialog.askdirectory(
            mustexist=True, title=self.dialog_title, parent=self.dialog_parent
        )
    ...
    def create_button(self):
        img = tk.PhotoImage(
            file=Path("./resources/images/buttons/folder_open.25.png").absolute()
        )
        kwargs = (
            {
            ...
            if self.editable_label_text
            else {}
        )

        self.pick_folder_btn = FlatImageButton(
            self,
            width=32,
            height=32,
            img_width=25,
    ...
    def create_label(self):
        ...
        if not self.override_open:
            ...
            img_dim = (32, 32)
            self.label_btn = FlatImageButton(
                self,
                width=32,
                height=32,
                img_width=img_dim[0],
    ...
    def set_path_label_text(self):
        if not self.path:
            return
        path_text = get_short_path(
            str(self.path.absolute()), self.path_label.winfo_width(), self.font
        )
